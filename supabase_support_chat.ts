...
Răspunzi în română, concis și prietenos. Exprimă-te corect gramatical și natural. Știi totul despre AutoAssist:
...
Nu fabrica informații despre lucruri din afara AutoAssist (proceduri oficiale, legi etc) — dacă nu ești sigur, spune că nu ești sigur și recomandă sursa oficială.`;
...
async function callGemini(messages: any[]): Promise<string> {
  ...
  const text = (data?.candidates?.[0]?.content?.parts || [])
    ...
    .join('')
    .trim();
  if (!text) throw new Error(data?.error?.message || 'Gemini: răspuns gol');
  return text;
}

async function callClaudeRaw(messages: any[]): Promise<string> {
...
async function callClaude(messages: any[]): Promise<{ reply: string; needsEscalation: boolean }> {
  ...
  if (GEMINI_KEY()) {
    try {
      ...
      console.error('Gemini a eșuat în support-chat, fallback pe Claude:', e);
    }
  }
  if (!text) {
    try {
      text = await callClaudeRaw(messages);
    } catch (e) {
      text = 'Îmi pare rău, a apărut o eroare.';
    }
  }
