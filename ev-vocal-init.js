...
function dismissOverlay() {
  ...
  if(overlay){ setTimeout(()=>{ overlay.style.opacity='0'; setTimeout(()=>overlay.style.display='none',200); }, wait); }
}

// Check auth on load
async function initAuth() {
  // Safety: ALWAYS dismiss overlay after max 2s, indiferent ce se întâmplă
  setTimeout(function(){
    const ov = document.getElementById('app-init-overlay');
    if(ov && ov.style.display !== 'none'){ ov.style.opacity='0'; setTimeout(()=>ov.style.display='none',200); }
  ...
  if(hash.includes('access_token') || hash.includes('refresh_token')) {
    ...
    if(type === 'recovery') {
      sessionStorage.removeItem('aa_recovery_type');
      history.replaceState({sec:'dashboard',idx:0}, '', window.location.pathname);
      setTimeout(()=>openM('reset-new-pass'), 400);
      return;
    }
    history.replaceState({sec:'dashboard',idx:0}, '', window.location.pathname);
    let session = null;
  ...
  // Show app after auth check — minimum 300ms splash vizibil (doar cât să nu clipească)
  dismissOverlay();
  ...
  supabaseClient.auth.onAuthStateChange(async (event, session) => {
    ...
    // Ignor reînnoirile automate de token și evenimentul inițial — altfel se reîncarcă tot în buclă
    if(event === 'TOKEN_REFRESHED' || event === 'INITIAL_SESSION' || event === 'USER_UPDATED') {
      if(session?.user) currentUser = session.user;
      return;
    }
    // Dacă userul e deja logat și primesc iar SIGNED_IN pentru același user, nu reîncarc nimic
    if(event === 'SIGNED_IN' && currentUser && session?.user?.id === currentUser.id) return;
...
function updateUserUI(user) {
  ...
  if(user) {
    ...
    if(user) setTimeout(()=>{if(typeof showDevBar==='function')showDevBar();}, 500);
  } else {
...
// ═══ PECO & CARBURANT ═══
async function loadFuelPrices(retry) {
  const body = document.getElementById('fuel-prices-body');
  if(!body) return;
  if(!supabaseClient) {
    // Clientul Supabase nu e încă inițializat — reîncerc de câteva ori
    if((retry||0) < 10) { setTimeout(()=>loadFuelPrices((retry||0)+1), 400); return; }
    body.innerHTML = '<div style="text-align:center;padding:20px;color:var(--t3)">Conexiune indisponibilă</div>';
    return;
  }
  try {
    const { data, error } = await supabaseClient.from('fuel_national').select('*').eq('id', 1).single();
    if(error || !data) throw error || new Error('fără date');

    const prices = [
      {name:'Benzină 95', avg:data.benzina_avg, min:data.benzina_min, icon:'🟢'},
      {name:'Motorină', avg:data.motorina_avg, min:data.motorina_min, icon:'🔵'},
      {name:'GPL', avg:data.gpl_avg, min:data.gpl_min, icon:'🟡'},
    ];
    body.innerHTML = prices.map(p => `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--b1)">
        ...
        <span style="text-align:right">
          <span style="font-weight:700;color:var(--text);font-size:15px">${p.avg?.toFixed(2) ?? '-'} RON/L</span>
          <span style="display:block;font-size:10px;color:var(--t3)">minim ${p.min?.toFixed(2) ?? '-'} RON/L</span>
        </span>
      </div>
    `).join('');

    const dataLabel = data.data ? new Date(data.data).toLocaleDateString('ro-RO') : '';
    body.innerHTML += `<div style="font-size:11px;color:var(--t3);margin-top:8px;text-align:center">Actualizat: ${dataLabel}</div>`;
    ...
    if(benzEl) benzEl.textContent = (data.benzina_avg?.toFixed(2) ?? '-') + ' RON/L';
    if(motorEl) motorEl.textContent = (data.motorina_avg?.toFixed(2) ?? '-') + ' RON/L';
    if(gplEl) gplEl.textContent = (data.gpl_avg?.toFixed(2) ?? '-') + ' RON/L';

    window._fuelAvgPrices = {benzina: data.benzina_avg, motorina: data.motorina_avg, gpl: data.gpl_avg};
  } catch(e) {
    console.error('Eroare încărcare prețuri carburant:', e);
    body.innerHTML = `<div style="text-align:center;padding:20px;color:var(--t3);font-size:12px">Prețurile nu sunt disponibile momentan<br><span style="font-size:10px;opacity:0.6">${e?.message || e}</span></div>`;
  }
}
...
function calcFuelCost() {
  ...
  const prices = window._fuelAvgPrices || {benzina: 7.05, motorina: 6.95, gpl: 3.45};
  const price = prices[type] || prices.benzina;
...
// Distanța dintre 2 puncte GPS (formula Haversine), rezultat în km
...
function findStations(type) {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    // Stații EV — nu avem date reale de preț, redirect Google Maps
    if(type !== 'gas_station') {
      container.innerHTML = `
        <div style="height:300px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;background:var(--s2);border-radius:12px">
          ...
          <div style="font-size:13px;color:var(--t2);text-align:center">Caută stații EV în zona ta</div>
          <a href="https://www.google.com/maps/search/statii+incarcare+electrice/@${lat},${lng},14z" target="_blank"
             class="btn btn-primary" style="font-size:13px;text-decoration:none">🗺️ Deschide în Google Maps</a>
        </div>`;
      if(list) list.innerHTML = '';
      return;
    }

    // Benzinării — date reale din Supabase (sincronizate săptămânal)
    if(!supabaseClient) { container.innerHTML = '<div style="padding:20px;text-align:center;color:var(--red)">Eroare conexiune</div>'; return; }

    const { data: statii, error } = await supabaseClient
      .from('fuel_stations')
      .select('*')
      .gte('lat', lat - 0.5).lte('lat', lat + 0.5)
      .gte('lon', lng - 0.5).lte('lon', lng + 0.5);

    if(error || !statii || !statii.length) {
      container.innerHTML = `
        <div style="height:200px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;background:var(--s2);border-radius:12px">
          <div style="font-size:32px">⛽</div>
          <div style="font-size:13px;color:var(--t3);text-align:center">Nu avem încă date pentru zona ta</div>
          <a href="https://www.google.com/maps/search/benzinarie/@${lat},${lng},14z" target="_blank" class="btn btn-ghost btn-sm" style="text-decoration:none">🗺️ Caută pe Google Maps</a>
        </div>`;
      if(list) list.innerHTML = '';
      return;
    }

    const apropiate = statii
      .map(s => ({...s, dist: distKm(lat, lng, s.lat, s.lon)}))
      .filter(s => s.dist <= 15)
      .sort((a,b) => a.dist - b.dist)
      .slice(0, 10);

    if(!apropiate.length) {
      container.innerHTML = '<div style="padding:20px;text-align:center;color:var(--t3)">Nicio benzinărie în raza de 15 km</div>';
      if(list) list.innerHTML = '';
      return;
    }

    container.innerHTML = `
      <div style="height:120px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;background:var(--s2);border-radius:12px">
        <div style="font-size:28px">⛽</div>
        <div style="font-size:13px;color:var(--t2)">${apropiate.length} benzinării găsite în apropiere</div>
      </div>`;

    if(list) list.innerHTML = apropiate.map(s => `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;background:var(--s2);border-radius:10px;margin-bottom:6px">
        <div>
          <div style="font-weight:700;font-size:13px">${s.nume || s.brand}</div>
          <div style="font-size:11px;color:var(--t3)">${s.dist.toFixed(1)} km${s.adresa ? ' · ' + s.adresa : ''}</div>
        </div>
        <div style="text-align:right">
          <div style="font-weight:700;font-size:13px;color:var(--accent)">${s.benzina_standard ? s.benzina_standard.toFixed(2)+' RON' : '-'}</div>
          <a href="https://www.google.com/maps/dir/?api=1&destination=${s.lat},${s.lon}&travelmode=driving" target="_blank" style="font-size:11px;color:var(--t3);text-decoration:underline">Traseu →</a>
        </div>
      </div>`).join('');
...
// Load fuel prices whenever secțiunea peco devine activă — indiferent cum (click, restaurare automată, link direct)
document.addEventListener('DOMContentLoaded', () => {
  let _fuelLoadedAt = 0;
  const safeLoadFuelPrices = () => {
    const now = Date.now();
    if(now - _fuelLoadedAt < 2000) return; // evit apeluri duplicate în aceeași fereastră de 2s
    _fuelLoadedAt = now;
    loadFuelPrices();
  };

  const origApply = window._applySection;
  if(origApply) {
    window._applySection = function(sec) {
      origApply(sec);
      if(sec === 'peco') setTimeout(safeLoadFuelPrices, 200);
    };
  }
  // Verificare de siguranță: dacă secțiunea peco e deja activă la încărcarea paginii (din orice motiv)
  setTimeout(() => {
    if(document.getElementById('sec-peco')?.classList.contains('active')) safeLoadFuelPrices();
  }, 800);
});
