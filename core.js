  ...
  el.innerHTML=cars.map(c=>{
    ...
    return`<div class="cc" style="transition:transform 0.15s,box-shadow 0.15s" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='';this.style.boxShadow=''">
      ...
      <div style="display:flex;gap:7px;flex-wrap:wrap">
        <button class="btn btn-primary btn-sm" style="flex:1" onclick="openCar(${c.id})">🚗 Detalii</button>
        <button class="btn btn-green btn-sm" onclick="goTo('asigurari')">🛡️ RCA</button>
        <button class="btn btn-ghost btn-sm" onclick="goTo('itp')">🔬 ITP</button>
        <button class="btn btn-ghost btn-sm" onclick="openVanz(${c.id})">💰 Vând</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderCarPage(car) {
  ...
  body.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <button class="btn btn-ghost btn-full" style="margin-top:12px;font-size:12px" onclick="openEditCar(${car.id})">✏️ Editează documente</button>
    </div>
