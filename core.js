...
function renderCarPage(car) {
  ...
  body.innerHTML = `
    ...
    <!-- Tip anvelope -->
    <div class="card" style="margin-bottom:16px">
      <div class="ch"><span class="ct">🛞 Tip anvelope</span></div>
      <div style="display:flex;gap:8px">
        <button class="btn ${!car.anvelope?.allSeason?'btn-primary':'btn-ghost'} btn-sm" onclick="setTireType(${car.id}, false)" style="flex:1">❄️☀️ Sezoniere</button>
        <button class="btn ${car.anvelope?.allSeason?'btn-primary':'btn-ghost'} btn-sm" onclick="setTireType(${car.id}, true)" style="flex:1">🔄 All-Season</button>
      </div>
      <div style="font-size:11px;color:var(--t3);margin-top:8px">${car.anvelope?.allSeason ? 'Nu primești alerte de schimbare sezonieră pentru această mașină.' : 'Primești alertă când e momentul să schimbi anvelopele de sezon.'}</div>
    </div>
