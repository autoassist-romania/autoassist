...
async function vanzEstimPret(){
  const c=cars.find(x=>x.id===selCarId);
  if(!c||!c.year)return;
  const hint=document.getElementById('vanz-pret-hint');
  if(!hint)return;
  ...
  const fuel=(document.getElementById('vanz-combustibil')||{}).value||c.fuel||'';
  const motor=(document.getElementById('vanz-motor')||{}).value||c.engine||'';
  hint.textContent='⏳ Se estimează prețul...';
  try {
    const resp=await fetch('https://zspcknjuqdjfxtqrqhhm.supabase.co/functions/v1/asistent',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        messages:[{role:'user',content:`Estimează prețul de piață în EUR pentru: ${brand} ${model} ${an}, ${km?Number(km).toLocaleString()+' km':'km necunoscut'}${fuel?' '+fuel:''}${motor?' '+motor+'cm³':''}, în România în ${new Date().getFullYear()}. Răspunde DOAR cu intervalul de preț în formatul: MIN-MAX EUR (ex: 8500-11000 EUR). Fără alte explicații.`}],
        system:'Ești expert în evaluarea autoturismelor pe piața din România. Cunoști prețurile reale de pe OLX, Autovit, Mobile.de. Răspunzi DOAR cu intervalul de preț cerut.'
      })
    });
    const data=await resp.json();
    const reply=(data.reply||'').trim();
    if(reply) hint.textContent=`💡 Estimare piață: ${reply}`;
