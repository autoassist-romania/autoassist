...
function vanzFotoPreview(input){
  ...
  files.slice(0, 6 - startSlot).forEach((file, i) => {
...
function renderVanzGalerie(){
  ...
  if(!el) return;
  if(!window._vanzFotos) window._vanzFotos = [];
  // Afișez 6 sloturi fixe
  el.innerHTML = Array.from({length:6}).map((_, i) => {
    const f = window._vanzFotos[i];
    return f
      ? `<div style="position:relative;height:90px;border-radius:8px;overflow:hidden;cursor:pointer" onclick="document.getElementById('vanz-foto-input').dataset.slot=${i};document.getElementById('vanz-foto-input').click()">
          <img src="${f}" style="width:100%;height:100%;object-fit:cover">
          <button onclick="event.stopPropagation();vanzStergeFoto(${i})" style="position:absolute;top:3px;right:3px;background:rgba(0,0,0,0.7);border:none;color:#fff;width:20px;height:20px;border-radius:50%;cursor:pointer;font-size:12px;line-height:1">×</button>
        </div>`
      : `<div style="height:90px;background:var(--s2);border:2px dashed var(--b2);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:var(--t3);gap:4px" onclick="document.getElementById('vanz-foto-input').dataset.slot=${i};document.getElementById('vanz-foto-input').click()">
          <span style="font-size:22px">📷</span>
          <span style="font-size:10px">Foto ${i+1}</span>
        </div>`;
  }).join('');
