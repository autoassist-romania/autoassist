...
async function exportRaportService(carId) {
  ...
  function checkY(need) { if(y + need > 275) addPage(); }
  ...
  doc.text('AutoAssist', PL + 2, 15);

  doc.setTextColor(180, 180, 200);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('www.autoassist.ro', PL + 2, 22);

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('RAPORT ISTORIC SERVICE', PL + 2, 34);

  // Data generare (dreapta)
  doc.setTextColor(140, 140, 160);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  const genDate = new Date().toLocaleDateString('ro-RO');
  doc.text('Generat: ' + genDate, W - PR, 34, { align: 'right' });

  y = 52;

  // ── CARD VEHICUL ──
  doc.setFillColor(240, 244, 255);
  doc.roundedRect(PL, y, TW, 36, 3, 3, 'F');
  doc.setDrawColor(79, 125, 255);
  doc.setLineWidth(0.5);
  doc.roundedRect(PL, y, TW, 36, 3, 3, 'S');

  // Accent stânga card
  doc.setFillColor(79, 125, 255);
  doc.roundedRect(PL, y, 3, 36, 1.5, 1.5, 'F');

  doc.setTextColor(20, 20, 50);
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(`${car.brand || ''} ${car.model || ''} ${car.year || ''}`, PL + 8, y + 10);

  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(80, 80, 110);
  doc.text(`Nr. inmatriculare: ${car.plate}`, PL + 8, y + 18);
  doc.text(`Kilometraj: ${car.km ? car.km.toLocaleString() + ' km' : '—'}`, PL + 8, y + 24);
  if(car.fuel) doc.text(`Combustibil: ${car.fuel}`, PL + 8, y + 30);
  if(car.vin) doc.text(`VIN: ${car.vin}`, PL + TW/2, y + 24);
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text(`${list.length}`, W - PR - 20, y + 12, { align: 'center' });
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.text('interventii', W - PR - 20, y + 18, { align: 'center' });
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(240, 180, 41);
  doc.text(`${totalCost.toLocaleString()}`, W - PR - 20, y + 28, { align: 'center' });
  doc.setFontSize(7);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(120, 120, 140);
  doc.text('RON total', W - PR - 20, y + 33, { align: 'center' });

  y += 44;

  // ── TITLU SECTIUNE ──
  doc.setFillColor(79, 125, 255);
  doc.rect(PL, y, TW, 8, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  doc.text('ISTORICUL INTERVENTIILOR (cronologic)', PL + 4, y + 5.5);
  y += 12;
  ...
  sorted.forEach((m, i) => {
    const isOdd = i % 2 === 0;
    const lines = [];
    if(m.detalii) lines.push('Lucrari: ' + m.detalii);
    if(m.obs) lines.push('Obs: ' + m.obs);
    // calcul inaltime rand
    let extraH = 0;
    lines.forEach(line => {
      const wrapped = doc.splitTextToSize(line, TW - 60);
      extraH += wrapped.length * 4;
    });
    const rowH = 16 + extraH;
    checkY(rowH + 2);

    // Fundal alternant
    doc.setFillColor(isOdd ? 248 : 255, isOdd ? 250 : 255, isOdd ? 255 : 255);
    doc.rect(PL, y, TW, rowH, 'F');

    // Linie separator
    doc.setDrawColor(220, 225, 240);
    doc.setLineWidth(0.2);
    doc.line(PL, y + rowH, PL + TW, y + rowH);

    // Număr intervenție (cerc albastru)
    doc.setFillColor(79, 125, 255);
    doc.circle(PL + 7, y + 8, 4, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text(String(i + 1), PL + 7, y + 10, { align: 'center' });

    // Tip intervenție
    doc.setTextColor(20, 20, 50);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(m.tip, PL + 14, y + 8);
    ...
    doc.setFont('helvetica', 'normal');
    let meta = m.data || '';
    if(m.km) meta += '  |  ' + Number(m.km).toLocaleString() + ' km';
    if(m.furnizor) meta += '  |  ' + m.furnizor;
    doc.text(meta, PL + 14, y + 14);

    // Cost (dreapta)
    if(m.cost) {
      doc.setFillColor(240, 180, 41);
      doc.roundedRect(W - PR - 28, y + 2, 26, 10, 2, 2, 'F');
      doc.setTextColor(30, 20, 0);
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text(Number(m.cost).toLocaleString() + ' RON', W - PR - 15, y + 8.5, { align: 'center' });
    }

    // Linii extra (detalii + obs)
    let ey = y + 17;
    lines.forEach(line => {
      doc.setTextColor(80, 80, 110);
      doc.setFontSize(7.5);
      doc.setFont('helvetica', 'italic');
      const wrapped = doc.splitTextToSize(line, TW - 20);
      doc.text(wrapped, PL + 14, ey);
      ey += wrapped.length * 4;
    });
    ...
    if(m.factura) {
      ...
      doc.circle(PL + TW - 4, y + 4, 2, 'F');
      doc.setTextColor(0, 160, 80);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      doc.text('F', PL + TW - 4, y + 5.5, { align: 'center' });
    }
  ...
  doc.rect(PL, y, TW, 14, 'F');
  doc.setTextColor(180, 180, 210);
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('TOTAL CHELTUIELI SERVICE', PL + 6, y + 9);
  doc.setTextColor(240, 180, 41);
  doc.setFontSize(13);
  doc.setFont('helvetica', 'bold');
  doc.text(totalCost.toLocaleString() + ' RON', W - PR, y + 9, { align: 'right' });
  y += 20;
  ...
  for(let p = 1; p <= pageCount; p++) {
    ...
    doc.setFontSize(7);
    doc.setFont('helvetica', 'normal');
    doc.text('AutoAssist — www.autoassist.ro', PL, 291);
    doc.text(`Pagina ${p} / ${pageCount}`, W - PR, 291, { align: 'right' });
  }

  const filename = `Raport_Service_${car.plate.replace(/\s/g,'_')}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
  showNotification('✅ PDF descărcat!', 'Raportul service a fost salvat ca PDF.');
}
