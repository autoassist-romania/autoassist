...
function showSeasonalAlert() {
  ...
  const icon = `<span style="font-size:18px">${trebuieIarna?'❄️':'☀️'}</span>`;
  ...
  const chipsHtml = cars.map(car => {
    ...
    let statusHtml;
    if(isAllSeason) {
      statusHtml = `<span style="font-size:11px;font-weight:700;color:var(--accent)">🔄</span>`;
    } else if(needsChange) {
      ...
      statusHtml = `<span style="font-size:11px;font-weight:700;color:var(--green)">✅</span>`;
    }
    return `<div style="display:inline-flex;align-items:center;gap:8px;padding:5px 10px;background:rgba(255,255,255,0.05);border-radius:20px">
      <span style="font-size:12px;font-weight:700">${car.plate}</span>
      ${statusHtml}
      <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;font-weight:600;color:${isAllSeason?'var(--accent)':'var(--t3)'}">
