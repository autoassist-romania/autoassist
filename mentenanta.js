...
function showSeasonalAlert() {
  ...
  const rowsHtml = cars.map(car => {
    ...
    let statusHtml = '';
    if(!isAllSeason) {
      statusHtml = needsChange
        ? `<button onclick="marcheazaSchimbatAnvelope('${sezonNecesar}','${car.id}')" class="btn btn-primary btn-sm" style="font-size:11px;padding:4px 10px">✅ Am schimbat</button>`
        : `<span style="font-size:12px;font-weight:700;color:var(--green)">✅ Schimbate</span>`;
    }
    return `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 10px;background:rgba(255,255,255,0.03);border-radius:8px">
      <span style="font-size:13px;font-weight:700">${car.plate}</span>
      <div style="display:flex;align-items:center;gap:12px">
        ${statusHtml}
        <label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:12px;font-weight:600;color:${isAllSeason?'var(--accent)':'var(--t2)'}">
          <input type="checkbox" ${isAllSeason?'checked':''} onchange="setTireType('${car.id}', this.checked)" style="width:16px;height:16px;cursor:pointer;accent-color:var(--accent)">
          All-Season
        </label>
      </div>
    </div>`;
  ...
  el.innerHTML = headerHtml + `<div style="display:flex;flex-direction:column;gap:4px">${rowsHtml}</div>`;
}
