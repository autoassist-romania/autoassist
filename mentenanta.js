...
function renderMntList() {
  ...
  const raportBtn = `<div style="display:flex;gap:8px;margin-bottom:12px">
    <button onclick="exportRaportService('${carId}')" class="btn btn-ghost btn-sm" style="flex:1;font-size:12px">📄 Raport Service (text)</button>
    <button onclick="copyRaportService('${carId}')" class="btn btn-ghost btn-sm" style="flex:1;font-size:12px">📋 Copiază raport</button>
  </div>`;
...
function openMntDetail(carId, id) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--s1);border-radius:20px 20px 0 0;padding:24px;width:100%;max-width:480px;max-height:85vh;overflow-y:auto">
      ...
      <div style="display:flex;gap:8px">
        <button onclick="delMntConfirm('${carId}',${m.id},this)" class="btn btn-sm" style="flex:1;background:rgba(255,71,87,0.15);color:var(--red);border:1px solid rgba(255,71,87,0.3)">🗑 Șterge</button>
        <button onclick="this.closest('div[style*=fixed]').remove()" class="btn btn-primary btn-sm" style="flex:2">✓ Închide</button>
      </div>
    </div>`;
...
function delMntConfirm(carId, id, btn) {
  btn.textContent = 'Sigur ștergi?';
  btn.onclick = () => { delMnt(carId, id); btn.closest('div[style*=fixed]').remove(); };
}
...
function exportRaportService(carId) {
  const result = _buildRaportService(carId);
  if(!result) { showNotification('❌ Eroare', 'Nu există intervenții de exportat.'); return; }
  const blob = new Blob([result.txt], {type:'text/plain;charset=utf-8'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `Raport_Service_${result.car.plate}_${new Date().toISOString().split('T')[0]}.txt`;
  a.click();
  URL.revokeObjectURL(url);
  showNotification('✅ Raport descărcat!', 'Fișierul a fost salvat pe dispozitivul tău.');
}
