...
function renderMntCarTabs() {
  const tabsEl = document.getElementById('mnt-car-tabs');
  if(!tabsEl) return;
  if(!cars || !cars.length) { tabsEl.innerHTML = ''; return; }

  // Dacă nu e niciun tab activ, selectează prima mașină
  if(!window._mntActiveCarId || !cars.find(c=>c.id==window._mntActiveCarId)) {
    // Preferă mașina din _returnToCarId dacă există
  ...
  tabsEl.innerHTML = cars.map(c => {
    ...
    return `<button onclick="mntSelectCar(${c.id})" style="
      ...
      ${count>0?`<span style="background:${isActive?'var(--accent)':'var(--s3)'};color:${isActive?'#fff':'var(--t3)'};font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;flex-shrink:0">${count}</span>`:''}
    </button>`;
  }).join('');

  // Sincronizează și select-ul ascuns (pentru compatibilitate cu funcțiile existente)
  const sel = document.getElementById('mnt-car-sel');
