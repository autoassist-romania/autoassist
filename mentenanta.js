...
function showSeasonalAlert() {
  ...
  const icon = `<span style="font-size:18px">${trebuieIarna?'❄️':'☀️'}</span><span style="font-size:12px;font-weight:700;color:var(--t2)">Anvelope ${trebuieIarna?'iarnă':'vară'}:</span>`;

  const chipsHtml = cars.map(car => {
    const isAllSeason = !!car.anvelope?.allSeason;
    const needsChange = masiniDeSchimbat.some(c=>c.id===car.id);
    let statusHtml;
    if(isAllSeason) {
      statusHtml = `<span style="font-size:11px;font-weight:700;color:var(--accent)">🔄 All-Season</span>`;
    } else if(needsChange) {
      statusHtml = `<button onclick="marcheazaSchimbatAnvelope('${sezonNecesar}','${car.id}')" class="btn btn-primary btn-sm" style="font-size:10px;padding:3px 8px">✅ Am schimbat</button>`;
    } else {
      statusHtml = `<span style="font-size:11px;font-weight:700;color:var(--green)">✅ Schimbate</span>`;
    }
    return `<div style="display:inline-flex;align-items:center;gap:8px;padding:5px 10px;background:${isAllSeason?'rgba(79,125,255,0.12)':'rgba(255,255,255,0.05)'};border-radius:20px;border:1px solid ${isAllSeason?'rgba(79,125,255,0.3)':'transparent'}">
      <span style="font-size:12px;font-weight:700;color:var(--text)">${car.plate}</span>
      ${statusHtml}
      <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;font-weight:700;color:var(--text)" title="Bifează dacă mașina are anvelope All-Season — nu vei mai primi alerte de schimbare sezonieră">
        <input type="checkbox" ${isAllSeason?'checked':''} onchange="setTireType('${car.id}', this.checked)" style="width:14px;height:14px;cursor:pointer;accent-color:var(--accent)">
        All-Season
      </label>
    </div>`;
  }).join('');
...
const DASH_ITEMS = [
  ...
  { id: 'detailing', icon: '🧽', label: 'Detailing', desc: 'Polish, folii, huse și îngrijire auto', gradient: 'linear-gradient(135deg,#00c8c8,#0088aa)', defaultOn: true },
