...
function mntGoToPiese() {
  // Sincronizează mașina selectată în Întreținere → Piese Auto
  if(window._mntActiveCarId) window._pieseCarId = window._mntActiveCarId;
  goTo('piese');
}
...
function populateMntSelects() {
  ...
  // Sincronizează _mntActiveCarId cu select-ul
  const sel = document.getElementById('mnt-car-sel');
  if(sel && sel.value) window._mntActiveCarId = sel.value;
  renderMntCarTabs();
  renderMntList();
}
...
// ID mașină activ în istoricul de service (independent de select)
window._mntActiveCarId = null;

function _renderCarTabsIn(containerId, onClickFn) {
  const tabsEl = document.getElementById(containerId);
  if(!tabsEl) return;
  if(!cars || !cars.length) { tabsEl.innerHTML = ''; return; }

  tabsEl.innerHTML = cars.map(c => {
    const isActive = String(c.id) === String(window._mntActiveCarId);
    const mntList = JSON.parse(localStorage.getItem('mnt_'+c.id)||'[]');
    const count = mntList.length;
    return `<button onclick="${onClickFn}(${c.id})" style="
      ...
      ${count>0?`<span style="
        background:${isActive?'var(--accent)':'var(--s3)'};
        color:${isActive?'#fff':'var(--t3)'};
        font-size:10px;font-weight:800;
        padding:2px 7px;border-radius:10px;flex-shrink:0;min-width:20px;text-align:center">${count}</span>`:''}
    </button>`;
  }).join('');
}

function renderMntCarTabs() {
  if(!window._mntActiveCarId || !cars.find(c=>c.id==window._mntActiveCarId)) {
    if(window._returnToCarId && cars.find(c=>c.id==window._returnToCarId)) {
      window._mntActiveCarId = window._returnToCarId;
    } else if(cars.length) {
      window._mntActiveCarId = cars[0].id;
    }
  }
  // Randează în ambele containere
  _renderCarTabsIn('mnt-car-tabs-top', 'mntSelectCar');
  _renderCarTabsIn('mnt-car-tabs', 'mntSelectCar');
  // Sincronizează select-ul ascuns
  const sel = document.getElementById('mnt-car-sel');
  if(sel) sel.value = window._mntActiveCarId;
}

function mntSelectCar(carId) {
  window._mntActiveCarId = carId;
  // Sincronizează ambele select-uri
  ['mnt-car-sel','rem-car-sel'].forEach(id => {
    const sel = document.getElementById(id);
    if(sel) sel.value = carId;
  });
  renderMntCarTabs();
  renderMntList();
  mntCarChange(); // actualizează hint-ul km în formular
}

function renderMntList() {
  // Determină carId activ — din tab sau din select ca fallback
  const carId = window._mntActiveCarId || document.getElementById('mnt-car-sel')?.value;
  const el = document.getElementById('mnt-list');
  if(!el) return;

  // Asigură că taburile sunt randate
  renderMntCarTabs();

  if(!carId) {
    el.innerHTML = '<div class="empty"><div class="ei">🔧</div><p>Nu ai mașini în garaj. Adaugă una din secțiunea Garaj.</p></div>';
    return;
  }
  ...
  if(!list.length) {
    el.innerHTML = `<div class="empty"><div class="ei">🔧</div><p>Nicio intervenție înregistrată pentru ${car?car.plate:'această mașină'}.<br><span style="font-size:12px;color:var(--t3)">Adaugă prima intervenție folosind formularul de mai sus.</span></p></div>`;
    renderReminderAuto();
    return;
  }
    ...
    <div onclick="openMntDetail('${carId}',${m.id})" style="background:var(--s2);border-radius:12px;padding:14px;margin-bottom:8px;border-left:3px solid var(--accent);cursor:pointer;transition:opacity 0.15s" onmouseover="this.style.opacity='0.85'" onmouseout="this.style.opacity='1'">
      <div style="display:flex;justify-content:space-between;align-items:start">
        <div style="flex:1">
          ...
          ${m.detalii?`<div style="font-size:11px;color:var(--t3);margin-top:4px;padding:6px;background:rgba(255,255,255,0.03);border-radius:6px">📝 ${m.detalii}</div>`:''}
          ${m.obs?`<div style="font-size:11px;color:var(--t3);margin-top:3px;padding:6px;background:rgba(255,255,255,0.03);border-radius:6px">${m.obs}</div>`:''}
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0;margin-left:12px">
          ...
          ${m.factura?`<span style="font-size:10px;background:rgba(0,200,100,0.12);color:var(--green);padding:2px 6px;border-radius:6px;border:1px solid rgba(0,200,100,0.2)">📎 factură</span>`:''}
          <div style="font-size:10px;color:var(--t3)">detalii →</div>
        </div>
      </div>
    </div>`).join('');
...
<div class="page">
  ...
  // Descarcă direct ca PDF fără dialog de printare
  ...
  try {
    ...
    // Randează HTML-ul într-un iframe ascuns
    const iframe = document.createElement('iframe');
    iframe.style.cssText = 'position:fixed;left:-9999px;top:0;width:794px;height:2000px;border:none;visibility:hidden;pointer-events:none';
    ...
    if(imgH <= pdfH) {
      ...
      let srcY = 0;
      const pageH = Math.floor((pdfH * canvas.width) / pdfW);
      while(srcY < canvas.height) {
        const sliceH = Math.min(pageH, canvas.height - srcY);
        const c2 = document.createElement('canvas');
        c2.width = canvas.width; c2.height = sliceH;
        c2.getContext('2d').drawImage(canvas, 0, srcY, canvas.width, sliceH, 0, 0, canvas.width, sliceH);
        if(srcY > 0) pdf.addPage();
        pdf.addImage(c2.toDataURL('image/jpeg',0.95), 'JPEG', 0, 0, pdfW, (sliceH*pdfW)/canvas.width);
        srcY += pageH;
      }
    }

    const filename = `Raport_Service_${car.plate.replace(/\s/g,'_')}_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(filename);
    showNotification('✅ PDF descărcat!', filename);
  } catch(e) {
    console.error('PDF error:', e);
    showNotification('❌ Eroare', 'Nu s-a putut genera PDF-ul. Încearcă din nou.');
  }
}
...
// Toggle permanent, reversibil — apelabil din pagina mașinii
function setTireType(carId, isAllSeason) {
  const car = cars.find(c=>c.id==carId);
  if(!car) return;
  if(!car.anvelope) car.anvelope = {};
  car.anvelope.allSeason = isAllSeason;
  save();
  if(typeof saveCarsToCloud === 'function') saveCarsToCloud();
  if(typeof showSeasonalAlert === 'function') showSeasonalAlert();
  if(typeof renderCarPage === 'function') renderCarPage(car);
  showNotification(
    isAllSeason ? '🔄 All-Season activat' : '❄️☀️ Anvelope sezoniere activate',
    isAllSeason ? `${car.plate} nu va mai primi alerte de schimbare sezonieră.` : `${car.plate} va primi din nou alerte de schimbare sezonieră.`
  );
}
...
function showSeasonalAlert() {
  ...
  if(!cars || !cars.length) { el.style.display='none'; el.innerHTML=''; return; }

  // Mașinile cu anvelope All-Season nu participă deloc la alerta sezonieră
  const masiniRelevante = cars.filter(c => !c.anvelope?.allSeason);
  if(!masiniRelevante.length) { el.style.display='none'; el.innerHTML=''; return; }

  // Verific dacă vreuna din mașini trebuie alertă
  const masiniDeSchimbat = masiniRelevante.filter(car => {
  ...
  if(!masiniDeSchimbat.length) {
    // Toate mașinile au anvelopele schimbate pentru sezonul curent — căsuță permanentă de status
    el.style.display = 'flex';
    el.style.flexDirection = 'column';
    el.style.alignItems = 'stretch';
    el.style.gap = '6px';
    el.style.background = 'rgba(0,232,154,0.08)';
    el.style.borderColor = 'rgba(0,232,154,0.25)';
    el.innerHTML = `
      <div style="display:flex;align-items:center;gap:12px">
        <span style="font-size:20px">${trebuieIarna?'❄️':'☀️'}</span>
        <div style="flex:1">
          <div style="font-weight:700;font-size:13px;color:var(--green)">Anvelope de ${trebuieIarna?'iarnă':'vară'} — schimbate ✅</div>
        </div>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:12px;padding-left:32px">
        ${masiniRelevante.map(c=>`<span onclick="setTireType('${c.id}', true)" style="font-size:11px;color:var(--t3);cursor:pointer;text-decoration:underline">${c.plate} are All-Season? →</span>`).join('')}
      </div>`;
    return;
  }

  // Notificare nativă — o singură dată per sezon/an, nu la fiecare încărcare de pagină
  const notifKey = `aa_seasonal_notif_${sezonNecesar}_${anCurent}`;
  if(!localStorage.getItem(notifKey)) {
    localStorage.setItem(notifKey, '1');
    const listaMasini = masiniDeSchimbat.map(c=>c.plate).join(', ');
    if(typeof showNotification === 'function') {
      showNotification(
        `${trebuieIarna?'❄️':'☀️'} Schimbă anvelopele de ${trebuieIarna?'iarnă':'vară'}!`,
        `${listaMasini} — sezonul ${trebuieIarna?'de iarnă':'de vară'} a început. Deschide AutoAssist pentru detalii.`
      );
    }
  }
  ...
  el.style.alignItems = 'stretch';
  el.style.gap = '6px';
  el.style.background = `linear-gradient(135deg,rgba(${trebuieIarna?'79,125,255':'240,180,41'},0.15),rgba(${trebuieIarna?'124,92,252':'255,140,0'},0.08))`;
  el.style.borderColor = `rgba(${trebuieIarna?'79,125,255':'240,180,41'},0.3)`;
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:12px">
      <span style="font-size:28px">${trebuieIarna?'❄️':'☀️'}</span>
      <div style="flex:1">
        <div style="font-weight:800;font-size:14px">Schimbă anvelopele de ${sezonLabel}!</div>
      ...
      <button onclick="marcheazaSchimbatAnvelope('${sezonNecesar}')" class="btn btn-primary btn-sm">✅ Am schimbat</button>
      <button onclick="document.getElementById('seasonal-banner').style.display='none'" style="background:none;border:none;color:var(--t3);cursor:pointer;font-size:18px;padding:0 4px">×</button>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:12px;padding-left:40px">
      ${masiniDeSchimbat.map(c=>`<span onclick="setTireType('${c.id}', true)" style="font-size:11px;color:var(--t3);cursor:pointer;text-decoration:underline">${c.plate} are All-Season? →</span>`).join('')}
    </div>`;
}
...
function marcheazaSchimbatAnvelope(sezon) {
  ...
  // Următoarea alertă REALĂ e pentru sezonul OPUS, nu pentru același sezon peste un an
  let dataUrmatoareaAlertaReala, labelUrmator;
  if (trebuieIarna) {
    // Am schimbat de iarnă -> următoarea alertă e de vară (1 aprilie)
    const anAlerta = luna <= 2 ? anCurent : anCurent + 1; // dacă suntem ian-mar, aprilie e tot anul ăsta
    dataUrmatoareaAlertaReala = new Date(anAlerta, 3, 1);
    labelUrmator = `1 aprilie ${anAlerta}`;
  } else {
    // Am schimbat de vară -> următoarea alertă e de iarnă (1 octombrie, mereu anul ăsta cât timp suntem apr-sep)
    dataUrmatoareaAlertaReala = new Date(anCurent, 9, 1);
    labelUrmator = `1 octombrie ${anCurent}`;
  }

  // Data după care se poate repeta alerta PENTRU ACELAȘI sezon (peste un an)
  const urmatoareaAlerta = trebuieIarna
    ? new Date(anCurent + 1, 9, 1).toISOString() // oct an viitor
    : new Date(anCurent + 1, 2, 1).toISOString(); // mar an viitor
  ...
  if(typeof saveCarsToCloud === 'function') saveCarsToCloud();
  showSeasonalAlert();
  showNotification('✅ Gata!', `Următoarea alertă (${trebuieIarna?'de vară':'de iarnă'}) apare din ${labelUrmator}.`);
}
...
// ═══ ANVELOPE 2.0 ═══
function anvLoadStatus() {
  const carId = document.getElementById('anv-car')?.value;
  const body = document.getElementById('anv-status-body');
  const empty = document.getElementById('anv-status-empty');
  if (!carId) {
  ...
  const anvData = car.anvData || {};
  const kmMontare = parseInt(anvData.kmMontare) || 0;
  const kmCurent = parseInt(car.km) || 0;
  const kmPeSet = kmMontare > 0 && kmCurent >= kmMontare ? kmCurent - kmMontare : 0;
  const kmTotal = 40000;
  const uzuraPct = Math.min(100, Math.round((kmPeSet / kmTotal) * 100));
  const kmRamasi = Math.max(0, kmTotal - kmPeSet);

  // km pe set
  const elKm = document.getElementById('anv-km-set');
  if (elKm) elKm.textContent = kmPeSet.toLocaleString('ro-RO');

  // uzură
  const elUz = document.getElementById('anv-uzura-pct');
  if (elUz) {
    elUz.textContent = kmPeSet > 0 ? uzuraPct + '%' : '—';
    elUz.style.color = uzuraPct > 80 ? 'var(--red)' : uzuraPct > 60 ? 'var(--gold)' : 'var(--green)';
  }

  // km rămași
  const elRam = document.getElementById('anv-km-ramasi');
  if (elRam) elRam.textContent = kmPeSet > 0 ? (kmRamasi / 1000).toFixed(0) + 'k' : '—';

  // bara uzură
  const bar = document.getElementById('anv-uzura-bar');
  if (bar) {
    bar.style.width = uzuraPct + '%';
    bar.style.background = uzuraPct > 80 ? 'var(--red)' : uzuraPct > 60 ? 'var(--gold)' : 'var(--green)';
  }

  // presiune
  const pf = anvData.presFata || '';
  const ps = anvData.presSpate || '';
  const elPfShow = document.getElementById('anv-pres-f-show');
  const elPsShow = document.getElementById('anv-pres-s-show');
  if (elPfShow) elPfShow.textContent = pf ? pf + ' bar' : '—';
  if (elPsShow) elPsShow.textContent = ps ? ps + ' bar' : '—';
  const inpF = document.getElementById('anv-pres-f');
  const inpS = document.getElementById('anv-pres-s');
  if (inpF) inpF.value = pf;
  if (inpS) inpS.value = ps;

  // montare
  const inpDt = document.getElementById('anv-dt-montare');
  const inpKm = document.getElementById('anv-km-montare');
  const inpMm = document.getElementById('anv-profil-mm');
  if (inpDt) inpDt.value = anvData.dataMontare || '';
  if (inpKm) inpKm.value = anvData.kmMontare || '';
  if (inpMm) inpMm.value = anvData.profilMm || '';
}

function anvSavePres() {
  const carId = document.getElementById('anv-car')?.value;
  if (!carId) return;
  const car = cars.find(c => c.id == carId);
  if (!car) return;
  if (!car.anvData) car.anvData = {};
  const pf = document.getElementById('anv-pres-f')?.value;
  const ps = document.getElementById('anv-pres-s')?.value;
  if (pf) { car.anvData.presFata = pf; document.getElementById('anv-pres-f-show').textContent = pf + ' bar'; }
  if (ps) { car.anvData.presSpate = ps; document.getElementById('anv-pres-s-show').textContent = ps + ' bar'; }
  _anvSaveToSupabase(car);
}

function anvSaveMontare() {
  const carId = document.getElementById('anv-car')?.value;
  if (!carId) return;
  const car = cars.find(c => c.id == carId);
  if (!car) return;
  if (!car.anvData) car.anvData = {};
  car.anvData.dataMontare = document.getElementById('anv-dt-montare')?.value || '';
  car.anvData.kmMontare = document.getElementById('anv-km-montare')?.value || '';
  car.anvData.profilMm = document.getElementById('anv-profil-mm')?.value || '';
  anvLoadStatus();
  _anvSaveToSupabase(car);
}

function anvResetSet() {
  const carId = document.getElementById('anv-car')?.value;
  if (!carId) return;
  const car = cars.find(c => c.id == carId);
  if (!car) return;
  ...
  car.anvData.dataMontare = today;
  car.anvData.kmMontare = car.km || 0;
  if (document.getElementById('anv-dt-montare')) document.getElementById('anv-dt-montare').value = today;
  if (document.getElementById('anv-km-montare')) document.getElementById('anv-km-montare').value = car.km || 0;
  anvLoadStatus();
  _anvSaveToSupabase(car);
  if (typeof showNotification === 'function') showNotification('✅ Set nou înregistrat!', 'Km de start: ' + (car.km || 0).toLocaleString('ro-RO'));
}

async function _anvSaveToSupabase(car) {
  const uid = typeof currentUser !== 'undefined' && currentUser?.id;
  if (!uid || typeof supabaseClient === 'undefined') {
    localStorage.setItem('autoassist-cars-' + uid, JSON.stringify(cars));
    return;
  }
  try {
    await supabaseClient.from('cars').update({ anvData: car.anvData }).eq('id', car.id).eq('user_id', uid);
    localStorage.setItem('autoassist-cars-' + uid, JSON.stringify(cars));
  } catch(e) { console.log('anvSave error:', e); }
}
