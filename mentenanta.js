...
function showSeasonalAlert() {
  ...
  if(!masiniDeSchimbat.length) { el.style.display='none'; el.innerHTML=''; return; }
...
function marcheazaSchimbatAnvelope(sezon) {
  ...
  const el = document.getElementById('seasonal-banner');
  if(el) {
    el.style.background = 'rgba(0,232,154,0.1)';
    el.style.borderColor = 'rgba(0,232,154,0.3)';
    el.innerHTML = `<span style="font-size:24px">✅</span>
      <div style="flex:1">
        ...
        <div style="font-size:12px;color:var(--t3)">Următoarea alertă (${trebuieIarna?'de vară':'de iarnă'}) apare din ${labelUrmator}.</div>
      </div>`;
    setTimeout(()=>{ if(el){ el.style.display='none'; el.innerHTML=''; } }, 5000);
  }
