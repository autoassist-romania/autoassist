...
function showSeasonalAlert() {
  ...
  const sezonLabel = trebuieIarna ? 'Iarnă' : 'Vară';

  if(!cars || !cars.length) { el.style.display='none'; el.innerHTML=''; return; }

  // Mașinile care chiar trebuie alertate (nu All-Season, nu deja schimbate pt sezonul curent)
  const masiniDeSchimbat = cars.filter(car => {
    if(car.anvelope?.allSeason) return false;
  ...
  // Notificare nativă — o singură dată per sezon/an
  ...
  const areAlerta = masiniDeSchimbat.length > 0;

  el.style.display = 'flex';
  el.style.flexDirection = 'column';
  el.style.alignItems = 'stretch';
  el.style.gap = '10px';
  el.style.background = areAlerta
    ? `linear-gradient(135deg,rgba(${trebuieIarna?'79,125,255':'240,180,41'},0.15),rgba(${trebuieIarna?'124,92,252':'255,140,0'},0.08))`
    : 'rgba(0,232,154,0.08)';
  el.style.borderColor = areAlerta ? `rgba(${trebuieIarna?'79,125,255':'240,180,41'},0.3)` : 'rgba(0,232,154,0.25)';

  const headerHtml = areAlerta
    ...
    : `<div style="display:flex;align-items:center;gap:12px">
        ...
        <div style="font-weight:700;font-size:13px;color:var(--green)">Anvelope de ${trebuieIarna?'iarnă':'vară'} — toate în regulă ✅</div>
      </div>`;

  const rowsHtml = cars.map(car => {
    const isAllSeason = !!car.anvelope?.allSeason;
    const needsChange = masiniDeSchimbat.some(c=>c.id===car.id);
    let statusHtml;
    if(isAllSeason) {
      statusHtml = `<span style="font-size:13px;font-weight:700;color:var(--accent)">🔄 All-Season</span>`;
    } else if(needsChange) {
      statusHtml = `<button onclick="marcheazaSchimbatAnvelope('${sezonNecesar}','${car.id}')" class="btn btn-primary btn-sm" style="font-size:12px">✅ Am schimbat</button>`;
    } else {
      statusHtml = `<span style="font-size:13px;font-weight:700;color:var(--green)">✅ Schimbate</span>`;
    }
    return `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;background:rgba(255,255,255,0.04);border-radius:10px;flex-wrap:wrap">
      <span style="font-size:14px;font-weight:700">${car.plate}</span>
      <div style="display:flex;align-items:center;gap:14px">
        ${statusHtml}
        <label style="display:flex;align-items:center;gap:7px;cursor:pointer;font-size:14px;font-weight:700;color:var(--text)">
          <input type="checkbox" ${isAllSeason?'checked':''} onchange="setTireType('${car.id}', this.checked)" style="width:19px;height:19px;cursor:pointer;accent-color:var(--accent)">
          All-Season
        </label>
      </div>
    </div>`;
  ...
  el.innerHTML = headerHtml + `<div style="display:flex;flex-direction:column;gap:8px">${rowsHtml}</div>`;
}
...
function marcheazaSchimbatAnvelope(sezon, carId) {
  ...
  let labelUrmator;
  if (trebuieIarna) {
    const anAlerta = luna <= 2 ? anCurent : anCurent + 1;
  ...
  const targetCars = carId ? cars.filter(c => c.id == carId) : cars;
