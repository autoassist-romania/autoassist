...
async function pieseFeedSearch() {
  ...
  resultsEl.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px">
    ${data.map(p => `
      <a href="${p.link_cumparare}" target="_blank" onclick="showCompatNotice()" style="text-decoration:none;background:var(--s2);border:1.5px solid var(--b2);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;transition:all 0.15s" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--b2)'">
...
// ═══ NOTIFICARE COMPATIBILITATE (apare în centrul paginii la fiecare click pe o piesă) ═══
function showCompatNotice() {
  document.getElementById('compat-notice')?.remove(); // resetez dacă apare din nou rapid
  ...
  el.innerHTML = `
    ...
    <button onclick="document.getElementById('compat-notice')?.remove()" style="
      margin-top:14px;padding:8px 22px;background:var(--accent);border:none;
      border-radius:10px;color:#fff;font-weight:700;font-size:12px;cursor:pointer">
      Am înțeles
    </button>
  `;
  document.body.appendChild(el);
  setTimeout(() => { document.getElementById('compat-notice')?.remove(); }, 6000);
}
