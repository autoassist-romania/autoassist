...
async function pieseFeedSearch() {
  ...
  if(!resultsEl || typeof supabase === 'undefined') return;
  ...
  let query = supabase.from('piese_automobilus').select('*').limit(60);
...
function populateFeedCatFilter() {
  ...
  sel.innerHTML = '<option value="">Toate categoriile</option>' +
    CATEGORII_PIESE.filter(c=>c.id!=='accesorii').map(c => `<option value="${c.id}">${c.icon} ${c.label}</option>`).join('');
}

async function populateFeedBrandFilter() {
  const sel = document.getElementById('feed-filter-brand');
  if(!sel || typeof supabase === 'undefined') return;
  const { data } = await supabase.from('piese_automobilus').select('brand').not('brand','is',null).limit(2000);
  ...
  el.innerHTML = `
    ...
    <div style="margin-top:16px;padding:12px 16px;background:var(--s2);border-radius:12px;display:flex;gap:10px;align-items:flex-start">
      ...
      <div style="font-size:11px;color:var(--t3);line-height:1.5">
        AutoDoc și Automobilus verifică amândouă compatibilitatea pieselor cu <strong>${car.brand} ${car.model} ${car.year||''}</strong>.
...
function pieseCopiazaVin(vin) {
  navigator.clipboard.writeText(vin).then(() => {
    showNotification('📋 VIN copiat!', `Lipește-l în căsuța de căutare VIN de pe site-ul magazinului (AutoDoc/Automobilus) — nu îl putem completa noi automat, e un widget al lor.`);
  }).catch(() => {
...
async function pieseCautaLiber() {
  ...
  // AutoDoc
  let autodocUrl = AUTODOC_BASE;
  if(brandSlug&&modelSlug) autodocUrl += `/${brandSlug}/${modelSlug}/?search=${encodeURIComponent(q)}`;
  else autodocUrl += `/cautare/?query=${encodeURIComponent(q)}`;
  if(AUTODOC_AFILIAT!=='AFILIAT_ID') autodocUrl += `&aff=${AUTODOC_AFILIAT}`;

  // Automobilus — fără search URL public confirmat, trimitem spre pagina mărcii (sau general)
  const automobilusUrl = buildAutomobilusUrl(car?.brand, null);

  // eMAG — căutare directă (fără tracking afiliat momentan, adăugăm când e activ Profitshare)
  const emagUrl = `https://www.emag.ro/search/${encodeURIComponent(q)}`;

  // Preț orientativ, dacă avem produsul în feed-ul nostru
  let produsGasit = null;
  if(typeof supabase !== 'undefined') {
    try {
      const { data } = await supabase.from('piese_automobilus').select('titlu,pret,brand')
        .ilike('titlu', `%${q}%`).order('pret', { ascending: true }).limit(1);
      if(data && data.length) produsGasit = data[0];
    } catch(e) { /* tabelul poate să nu existe încă — nu blocăm căutarea din cauza asta */ }
  }

  showStoreChooser(q, produsGasit, [
    { nume: 'AutoDoc',        icon: '🔵', url: autodocUrl },
